...
public class EnvironmentApplicationService {
 ...
 @Transactional public Environment create(EnvironmentCommand c){return environments.save(Environment.create(version(c.releaseVersionId()),c.type(),c.name(),c.host(),c.sshPort(),c.password(),c.workDirectory(),c.architecture(),c.mae(),c.maeUser(),c.maePassword(),c.osmu(),c.osmuUser(),c.osmuPassword()));}
 @Transactional public Environment update(Long id,EnvironmentCommand c){Environment e=environment(id);if(c.version()==null||!c.version().equals(e.getVersion()))throw new EnvironmentConflictException();e.update(version(c.releaseVersionId()),c.type(),c.name(),c.host(),c.sshPort(),c.password(),c.workDirectory(),c.architecture(),c.mae(),c.maeUser(),c.maePassword(),c.osmu(),c.osmuUser(),c.osmuPassword());return environments.save(e);}
 ...
 @Transactional public ConnectionTestResult test(Long id){Environment e=environment(id);ConnectionTestResult result=ssh.test(new ConnectionTestCommand(e.getType(),e.getHost(),e.getSshPort(),e.getPassword()));e.markTest(result.status(),result.latencyMs(),result.error());environments.save(e);return result;}
 public List<ConnectionTestResult> testAll(){return environments().stream().map(environment->test(environment.getId())).toList();}
