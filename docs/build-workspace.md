...
- 部署候选产物只来自仍然存在的成功单分支任务；删除历史任务时同步删除对应产物记录，部署接口也会拒绝孤立产物。
- 构建页通过当前构建环境实时读取其“工作目录”（例如 `/user1/wytest`）的目录占用和所在文件系统可用空间，不使用硬编码路径。
- 实时日志支持关键字搜索和复制；日志区在刷新状态时保持当前位置，停留在底部时自动跟随新输出。浏览器刷新会恢复最近查看的构建任务和仍在运行的 SSE 日志流。
