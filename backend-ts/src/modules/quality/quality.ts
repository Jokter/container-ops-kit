...
export class QualityService{
 private active=new Map<string,Promise<void>>();private controllers=new Set<AbortController>();private closing=false;
 ...
 list(){return this.store.records<QualityJob>('quality-job');}get(id:string){const job=this.store.getRecord<QualityJob>('quality-job',id);if(!job)throw Object.assign(new Error('查询任务不存在'),{statusCode:404});return job;}
 ...
 private async execute(job:QualityJob){let next=0;await Promise.all(Array.from({length:job.connection.parallel},async()=>{while(next<job.parts.length){const part=job.parts[next++]!;part.status='RUNNING';part.message='正在查询 Grafana';this.save(job);const controller=new AbortController();this.controllers.add(controller);const timeout=setTimeout(()=>controller.abort(),job.connection.timeoutSeconds*1000);
 ...
 }catch(error){part.status=this.closing?'INTERRUPTED':'FAILED';part.message=controller.signal.aborted?'查询超时或服务停止':error instanceof Error&&/^(Grafana|报告|后端|服务)/.test(error.message)?error.message:'Grafana 请求失败，请检查网络或响应格式';}
 finally{clearTimeout(timeout);this.controllers.delete(controller);this.logs.task('quality',job.id,{time:new Date().toISOString(),version:part.version,kind:part.kind,status:part.status,message:part.message});this.save(job);}
 }}));const failed=job.parts.filter(p=>['FAILED','INTERRUPTED'].includes(p.status)).length;job.status=this.closing?'INTERRUPTED':failed===job.parts.length?'FAILED':failed?'PARTIAL':'SUCCEEDED';this.save(job);}
