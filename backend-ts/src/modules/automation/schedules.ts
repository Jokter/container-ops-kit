...
export const scheduleInput=z.object({name:z.string().trim().min(1).max(100),enabled:z.boolean(),timing,task:z.discriminatedUnion('kind',[
 ...
 z.object({kind:z.literal('auto-ut'),config:reportConfig}),csvTask
...
export class UnifiedSchedules{
 ...
 refreshRuns(){for(const r of this.runs().filter(r=>r.status==='RUNNING'&&r.taskIds.length)){const tasks=this.autoUt.tasks().filter(t=>r.taskIds.includes(t.id));if(tasks.length!==r.taskIds.length)continue;if(tasks.some(t=>['DISCOVERED','PREPARING','BASELINE_RUNNING','REPAIRING','VERIFYING','PR_CREATING','WAITING_CONFIRMATION'].includes(t.status)))continue;r.status=tasks.every(t=>t.status==='RESOLVED')?(r.queryPartial?'PARTIAL':'SUCCEEDED'):'FAILED';r.message=r.status==='SUCCEEDED'?'全部修复任务已完成，MR 已创建':r.status==='PARTIAL'?'成功版本的修复已完成，部分版本报告查询失败':'部分任务需要人工处理，请打开关联任务';r.finishedAt=new Date().toISOString();this.saveRun(r);}}
