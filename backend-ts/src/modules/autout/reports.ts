...
export interface PlanItem{version:string;repository:string;failedTests:number;lineCoverage:number;lineGoal:number;branchCoverage:number;branchGoal:number;baseBranch:string;repositoryUrl:string;repositoryCustomized:boolean;configured:boolean;repairBranch:string;}
export interface ReportRun{id:string;jobId:string;trigger:'MANUAL'|'SCHEDULE';status:'FETCHING'|'READY'|'PARTIAL'|'FAILED'|'INTERRUPTED';createdAt:string;config:ReportConfig;plan:PlanItem[];taskIds:string[];messages:string[];claimed:string[];versionResults?:Record<string,string>;}
...
export class AutoUtReports{
 ...
 versionLocked(version:string,baseBranch:string){return this.autoUt.tasks().some(t=>t.reportVersion===version||!t.reportVersion&&t.baseBranch===baseBranch)||this.autoUt.archivedMrBlockers().some(t=>t.reportVersion===version||!t.reportVersion&&t.baseBranch===baseBranch);}
 assertCanCreate(id:string,version:string,username:string){
  ...
  if(!entry||!current||current.baseBranch!==entry.baseBranch||configured.username!==username)throw Object.assign(Error('版本或执行配置已变化，请保存配置并重新获取该版本报告。'),{statusCode:409});
  if(this.versionLocked(version,entry.baseBranch))throw Object.assign(Error('该版本已有 UT 修复任务，不能重复建单或刷新报告。'),{statusCode:409});
  const part=this.quality.get(run.jobId).parts.find(p=>p.version===version&&p.kind==='ut');
  if(!['READY','PARTIAL'].includes(run.status)||part?.status!=='SUCCEEDED'||!run.plan.some(p=>p.version===version))throw Object.assign(Error('该版本没有已确认的待治理仓库，请先成功获取该版本异常报告。'),{statusCode:409});
  return run;
 }
 bindTicket(id:string,version:string,username:string,ticket:string){
  const run=this.get(id),record=this.store.records<{username:string;version?:string;ticket:string;status:string}>('dts-ticket').find(t=>t.username===username&&t.version===version&&t.ticket===ticket);
  if(record?.status!=='READY')throw Object.assign(Error('问题单尚未确认流转成功，不能用于自动启动。'),{statusCode:409});
  if(!run.config.versions.some(v=>v.version===version))throw Object.assign(Error('报告不包含该版本'),{statusCode:409});
  const saved=this.configuration();if(saved.config.username!==username)throw Object.assign(Error('执行用户已变化，请核对问题单'),{statusCode:409});
  const row=saved.config.versions.find(v=>v.version===version);if(!row)throw Object.assign(Error('该版本已移除'),{statusCode:409});row.ticket=ticket;
  this.store.putRecord('auto-ut-report-config','main',saved);
 ...
 private async complete(run:ReportRun){try{const job=await this.quality.wait(run.jobId);if(this.closed||this.deleting.has(run.id)){run.status='INTERRUPTED';this.save(run);return;}
  run.versionResults=Object.fromEntries(job.parts.filter(p=>p.kind==='ut').map(p=>[p.version,p.status]));run.plan=this.plan(job,run.config);run.status=job.status==='FAILED'?'FAILED':job.status==='INTERRUPTED'?'INTERRUPTED':job.status==='PARTIAL'?'PARTIAL':'READY';run.messages=job.parts.map(p=>`${p.version}：${p.message}`);this.save(run);
 ...
 private plan(job:QualityJob,config:ReportConfig):PlanItem[]{const plan:PlanItem[]=[];for(const part of job.parts){if(part.status!=='SUCCEEDED')continue;const baseBranch=config.versions.find(v=>v.version===part.version)!.baseBranch;
  for(const row of part.rows){if(String(row['语言']).toLowerCase()!=='java'||row['PL组']!=='Access_智能驾舱组')continue;const repository=String(row['代码仓']);if(!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(repository))throw new Error('代码仓名称无效');const mapping=this.store.getRecord<{url:string}>('auto-ut-repository',repository.toLowerCase());
   if(!(metric(row['失败用例'])>0||metric(row['行覆盖率'],true)<Math.min(metric(row['行覆盖率目标'],true),.8)||metric(row['分支覆盖率'],true)<Math.min(metric(row['分支覆盖率目标'],true),.7)))continue;
 ...
 private async startWork(id:string,mode:'MANUAL'|'AUTOMATIC',selected?:string[],useCurrentConfig=false){
  ...
  try{const items=run.plan.filter(p=>!selected||selected.includes(`${p.version}/${p.repository}`));
   for(const item of items){if(this.deleting.has(id))break;const key=`${item.version}/${item.repository}`;if(run.claimed.includes(key)||this.autoUt.tasks().some(t=>t.repository.toLowerCase()===item.repository.toLowerCase()&&(t.reportVersion===item.version||!t.reportVersion&&t.baseBranch===item.baseBranch)))continue;
