...
export class AutoUtReports{
 ...
 }catch{run=this.get(run.id);run.messages.push('报告处理或自动修复启动失败，请检查配置和工作目录');if(run.status==='FETCHING')run.status='FAILED';this.save(run);}finally{this.logs.task('auto-ut',run.id,{time:new Date().toISOString(),status:run.status,messages:run.messages});}}
