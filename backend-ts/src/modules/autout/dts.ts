import type {AutoUtReports} from './reports.js';
...
export interface TicketResult {requestId:string;username:string;ticket:string;version?:string;product?:DtsProduct;status:'CREATING'|'DRAFT'|'READY'|'REVIEW'|'CANCELLED'|'CLOSED';message:string;stage?:'CREATE'|'FLOW'|'CONFIRM';nodeStatus?:string;nodeName?:string;currentHandler?:string}
...
export class DtsTickets{
 ...
 private async query(call:Call,value:TicketResult){
  ...
  value.status=['撤销','已撤销'].includes(value.nodeName.trim())?'CANCELLED':['关闭','已关闭'].includes(value.nodeName.trim())?'CLOSED':value.nodeStatus==='DTS009'&&handlerMatches?'READY':'REVIEW';
  value.message=['CANCELLED','CLOSED'].includes(value.status)?'问题单 '+value.ticket+(value.status==='CLOSED'?' 已关闭':' 已撤销')+'，不再复用；请重新点击自动建单。'+summary:value.status==='READY'?'问题单已流转到开发人员实施修改。'+summary:'问题单尚未确认流转到指定开发人员。'+summary;
  this.save(value);return {nodeStatus:value.nodeStatus,ready:value.status==='READY',terminal:['CANCELLED','CLOSED'].includes(value.status)};
 }
  ...
  try{
   for(let attempt=0;attempt<3;attempt++){
    ...
    try{
     ...
     if(confirmed.ready||confirmed.terminal)return;
 ...
 async control(ticket:string,username:string,action:'check'|'continue',version:string){
  ...
  try{
   ...
   if(action==='continue'&&status==='DTS001'&&!['CANCELLED','CLOSED'].includes(value.status))await this.execute(call,value);
