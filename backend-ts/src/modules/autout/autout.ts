...
import {readFile,writeFile,mkdir,stat,lstat,realpath,readdir,unlink,rm} from 'node:fs/promises';
...
import {runProcess,displayCommand} from '../../infrastructure/process.js';
...
const settings={language:'Java',plGroup:'Access_智能驾舱组',logDirectory:resolve(process.env.PLATFORM_LOG_DIR?.trim()||'data/logs','auto-ut','details'),piCommand:'pi',thinkingLevel:'medium',piTimeoutMs:30*60_000,maxAttempts:5,
...
export class AutoUtService{
 ...
 private async trackOne(task:AutoUtTask,checkOnly=false){task=this.get(task.id);if(this.running.has(task.id)||this.deleting.has(task.id)||this.closing)return;this.running.add(task.id);this.controllers.set(task.id,new AbortController());const execution=(async()=>{try{if(checkOnly)await this.mrWorkflow.checkTerminal(task);else await this.mrWorkflow.tick(task);}finally{await this.cleanupCompletedWorkspace(task);this.running.delete(task.id);this.controllers.delete(task.id);this.executions.delete(task.id);}})();this.executions.set(task.id,execution);await execution;}
 async controlMr(id:string,action:'pause'|'resume'|'retry'|'apply-settings'|'retry-notifications'|'check'|'rerun-pipeline'){
  ...
  const execution=this.controlMrAction(id,action).finally(()=>this.cleanupCompletedWorkspace(this.get(id)));this.executions.set(id,execution.then(()=>{},()=>{}));
 ...
 blocksRepository(repository:string,version:string,baseBranch:string){return this.tasks().some(t=>t.repository.toLowerCase()===repository.toLowerCase()&&(t.reportVersion===version||!t.reportVersion&&t.baseBranch===baseBranch)&&(!['RESOLVED','MR_CLOSED','NO_CHANGE'].includes(t.status)||this.running.has(t.id)))||this.governanceRecords().some(r=>r.repository.toLowerCase()===repository.toLowerCase()&&(r.reportVersion===version||!r.reportVersion&&r.baseBranch===baseBranch)&&r.governance.mrState==='PENDING'&&!r.mrBlockRelease)||this.tasks().some(t=>!t.governance&&t.pullRequestUrl&&t.repository.toLowerCase()===repository.toLowerCase()&&t.reportVersion===version);}
 ...
 private taskView(task:AutoUtTask){task.maxRepairAttempts=settings.maxAttempts;if(!task.compileFailure&&task.status==='WAITING_EXTERNAL'&&task.nextStage==='BASELINE'&&task.message.startsWith('刷新Maven依赖与预编译失败'))task.compileFailure={command:defaultCompileCommand.join(' ')};task.workspacePath=autoUtWorkspace(task);return task;}
 ...
 private schedule(task:AutoUtTask,repository:Repository){
  ...
  }).finally(async()=>{await this.cleanupCompletedWorkspace(task);this.running.delete(task.id);this.controllers.delete(task.id);this.executions.delete(task.id);});
 ...
 private async execute(task:AutoUtTask,repository:Repository){const workspace=autoUtWorkspace(task);task.governance??={mode:'NONE',coverageLow:false,maxClasses:5};task.governance.startedAt??=new Date().toISOString();this.save(task);
  try{do{const stage=task.nextStage;if(!this.claim(task))return;if(stage==='PREPARE'){await this.prepare(task,repository,workspace);this.wait(task,'BASELINE',`工作区准备完成：${workspace}`,20);}else if(stage==='BASELINE'){const evidence=await this.baseline(task,repository,workspace);this.change(task,task.status,`基线 UT：共 ${evidence.tests}，失败 ${evidence.failures+evidence.errors}，跳过 ${evidence.skipped}`);task.governance.baseline=evidence;task.governance.mode=decideGovernance(evidence,task.governance.coverageLow);
    ...
    }else if(stage==='REPAIR')await this.repair(task,workspace);else if(stage==='VERIFY')await this.verify(task,repository,workspace);else if(stage==='PUBLISH')await this.publish(task,workspace);this.save(task);if(task.governance?.lastFailure&&task.nextStage==='REPAIR'&&['WAITING_CONFIRMATION','RETRY_PENDING'].includes(task.status))await this.notifyTaskFailure(task);}while(task.executionMode==='AUTOMATIC'&&task.status==='WAITING_CONFIRMATION');}
  catch(error){this.change(task,'WAITING_EXTERNAL',error instanceof Error?error.message:'Auto-UT 执行异常');this.save(task);if(!this.closing&&!this.deleting.has(task.id))await this.notifyTaskFailure(task);}}
 private async rememberClonedWorkspace(task:AutoUtTask){
  const workspace=autoUtWorkspace(task),info=await lstat(workspace);
 ...
 private async cleanupCompletedWorkspace(task:AutoUtTask){
  ...
  try{
   workspace=autoUtWorkspace(task);
   if(this.cleaningWorkspaces.has(workspace)){result('SKIPPED','仓库正在清理，已跳过本次清理。');return;}
   this.cleaningWorkspaces.add(workspace);locked=true;
   task.workspaceCleanup={state:'RUNNING',message:'正在清理已完成任务的克隆仓库。'};this.save(task);
   const shared=this.tasks().some(other=>other.id!==task.id&&autoUtWorkspace(other)===workspace&&(!['RESOLVED','NO_CHANGE','MR_CLOSED'].includes(other.status)||this.running.has(other.id)));
   if(shared){result('SKIPPED','仓库仍被其他任务使用，已保留工作目录。');return;}
   const owned=this.store.getRecord<{root:string;path:string;dev:number;ino:number}>('auto-ut-workspace',workspace);
   if(!owned){result('SKIPPED','工作目录缺少工具克隆归属记录，已保留，请按需手动清理。');return;}
   const root=resolve(task.workspaceRoot),boundary=relative(root,workspace);
   if(!boundary||boundary.startsWith('..'))throw Error('工作目录边界校验失败');
   const logBoundary=relative(workspace,settings.logDirectory);if(!logBoundary||(!logBoundary.startsWith('..')&&!logBoundary.includes(':')))throw Error('工作目录包含日志目录');
   const info=await lstat(workspace).catch(error=>{if(error?.code==='ENOENT')return undefined;throw error;});
   if(info){
    const actualRoot=await realpath(root),actualWorkspace=await realpath(workspace);
    if(info.isSymbolicLink()||!info.isDirectory()||actualRoot!==owned.root||actualWorkspace!==owned.path||actualWorkspace!==resolve(actualRoot,boundary)||info.dev!==owned.dev||info.ino!==owned.ino)throw Error('工作目录归属或实际路径已变化');
    await rm(workspace,{recursive:true,force:true});
   }
   this.store.deleteRecord('auto-ut-workspace',workspace);
   result('DELETED','治理已完成，克隆仓库已清理；执行记录与日志已保留。');
  }catch{result('FAILED','治理已完成，但仓库清理失败，请检查目录权限、占用或路径变化后手动清理。');}
  finally{if(locked)this.cleaningWorkspaces.delete(workspace);}
 }
 private async notifyTaskFailure(task:AutoUtTask){
  const recovering=task.status==='WAITING_CONFIRMATION'&&task.executionMode==='AUTOMATIC';
  const reason=task.governance?.lastFailure&&recovering?task.governance.lastFailure:task.message;
 ...
 private async command(task:AutoUtTask,command:string[],directory:string,timeout:number,label:string,required=true){this.controllers.get(task.id)?.signal.throwIfAborted();const id=randomUUID();this.emit(task,'operation_start',label,id,displayCommand(command));const safe=label.replace(/[^\p{L}\p{N}._-]/gu,'_');try{const result=await runProcess(command,directory,timeout,join(settings.logDirectory,task.id,`${safe}.log`),undefined,undefined,this.controllers.get(task.id)?.signal);this.emit(task,'operation_end',label,id,'',result.exitCode!==0);if(required&&result.exitCode!==0)throw new Error(`${label}失败，退出码 ${result.exitCode}：${result.output.slice(0,2000)}`);return result;}catch(error){this.emit(task,'operation_end',label,id,'',true);throw error;}}
 ...
 private async baseline(task:AutoUtTask,repository:Repository,workspace:string){
  ...
  catch(error){task.compileFailure={command:command.join(' ')};this.save(task);throw error;}
  return(await this.testEvidence(task,repository.testCommand,workspace,'核验实际UT')).evidence;
 }
 ...
 private async testEvidence(task:AutoUtTask,command:string[],workspace:string,label:string){
  ...
  const result=await this.command(task,taskMavenCommand(command,task.mavenSelection),workspace,1800000,label,false);
 ...
 private async selectTargets(task:AutoUtTask,workspace:string){
  ...
  for(const file of sources){if(task.mavenSelection?.[1]&&!task.mavenSelection[1].split(',').some(module=>relative(workspace,file).replaceAll('\\','/').startsWith(module+'/')))continue;const name=file.split(/[\\\\/]/).at(-1)!.slice(0,-5);if(/(?:Dto|DTO|Vo|VO|Config|Configuration|Exception|Entity|Constants)$/.test(name)||texts.some(text=>new RegExp('\\b'+name+'\\b').test(text)))continue;
 ...
 private async repair(task:AutoUtTask,workspace:string){if(task.governance?.mode==='SUPPLEMENT'){await this.supplement(task,workspace);return;}const evidence=await this.readSurefire(workspace),beforeEvidence=task.governance?.verified??task.governance?.baseline;const failureClasses=[...new Set([...evidence.failedIds,...(beforeEvidence?.failedIds??[]),...(beforeEvidence?.caseIds.filter(id=>!evidence.caseIds.includes(id))??[])].map(testClass))];const dir=join(settings.logDirectory,task.id);await mkdir(dir,{recursive:true});const prompt=join(dir,`第${task.attempts}轮提示词.md`);await writeFile(prompt,[
  ...
  '不要运行 codecovcli 或 JaCoCo。针对失败类的验证命令：'+targetedTestCommand(taskMavenCommand(testCommand,task.mavenSelection),failureClasses.length?failureClasses:evidence.caseIds.map(testClass)).join(' '),
 ...
 private async supplement(task:AutoUtTask,workspace:string){
  ...
  for(const target of g.targets??[]){
   ...
   await writeFile(prompt,[
    ...
    '不要运行 codecovcli 或 JaCoCo，不要执行 clean 或全量测试。用 mvn -B -ntp -s .ci/settings.xml test -Djacoco.skip=true -Dtest=实际测试类全限定名 -Dsurefire.failIfNoSpecifiedTests=false '+(task.mavenSelection?.join(' ')||'')+' 验证当前类；平台最终统一执行所选模块全量回归。',
...
await app.register(multipart);const id=(p:unknown)=>z.object({id:z.uuid()}).parse(p).id;
 ...
 app.post('/api/auto-ut/tasks/:id/compile-retry',async(req,reply)=>{const body=z.object({command:z.string().min(1).max(4096)}).strict().parse(req.body);return reply.code(202).send(await service.retryCompile(id(req.params),body.command));});
