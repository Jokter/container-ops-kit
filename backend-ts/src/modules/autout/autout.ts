...
export class AutoUtService{
 ...
 private taskView(task:AutoUtTask){if(!task.compileFailure&&task.status==='WAITING_EXTERNAL'&&task.nextStage==='BASELINE'&&task.message.startsWith('刷新Maven依赖与预编译失败'))task.compileFailure={command:defaultCompileCommand.join(' ')};task.workspacePath=autoUtWorkspace(task);return task;}
