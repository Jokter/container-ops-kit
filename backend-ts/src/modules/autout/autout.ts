...
export class AutoUtService{
 ...
 governanceSummary(version='',days=0){const cutoff=days?Date.now()-days*86400000:0;const records=this.governanceRecords().filter(r=>(!version||r.reportVersion===version)&&Date.parse(r.createdAt)>=cutoff);return{metrics:governanceMetrics(records),records};}
 resolveMr(id:string,state:'MERGED'|'CLOSED'){const record=this.store.getRecord<GovernanceRecord>('auto-ut-governance',id);if(!record?.pullRequestUrl||record.status!=='RESOLVED')throw Object.assign(new Error('没有待确认的已完成 MR'),{statusCode:409});record.governance.mrState=state;record.updatedAt=new Date().toISOString();this.store.putRecord('auto-ut-governance',id,record,record.createdAt);const task=this.store.getRecord<AutoUtTask>('auto-ut-task',id);if(task?.governance){task.governance.mrState=state;this.save(task);}return record;}
 blocksRepository(repository:string,version:string,baseBranch:string){return this.tasks().some(t=>t.repository.toLowerCase()===repository.toLowerCase()&&(t.reportVersion===version||!t.reportVersion&&t.baseBranch===baseBranch)&&t.status!=='RESOLVED')||this.governanceRecords().some(r=>r.repository.toLowerCase()===repository.toLowerCase()&&(r.reportVersion===version||!r.reportVersion&&r.baseBranch===baseBranch)&&r.governance.mrState==='PENDING')||this.tasks().some(t=>!t.governance&&t.pullRequestUrl&&t.repository.toLowerCase()===repository.toLowerCase()&&t.reportVersion===version);}
 ...
 private completeWithoutChanges(task:AutoUtTask,message:string){task.nextStage='DONE';task.progress=100;if(task.governance)task.governance.completedAt=new Date().toISOString();this.change(task,'RESOLVED',message);}
 ...
 private async publish(task:AutoUtTask,workspace:string){if(!settings.createMr)throw new Error('CodeHub MR 创建已被配置关闭。');const title=autoUtCommitTitle(task.ticket,task.repository);
  ...
  const command=[settings.codehub,'mr','upload','--dest',task.baseBranch,'--br',task.repairBranch,'--topic',task.repairBranch,'-T',title,'-D',`实际修复 ${task.governance?.fixedIds?.length??0} 个失败用例，新增 ${task.governance?.addedIds?.length??0} 个通过用例。\n\n完整 UT 回归通过；覆盖率以合并后质量报告为准。`];const recipients:Array<[string,string]>=[['--reviewers',settings.reviewers],['--approvers',settings.approvers],['--assignees',settings.assignees]];for(const[o,v]of recipients)if(v)command.push(o,v);command.push('-y');const uploaded=await this.command(task,command,workspace,600000,'创建CodeHub-MR');let mr:{id?:unknown;mr_url?:unknown}|undefined;for(const line of [uploaded.output,...uploaded.output.split(/\r?\n/).reverse()])try{const value=JSON.parse(line) as typeof mr;if(value?.id&&value.mr_url){mr=value;break;}}catch{/* next */}if(!mr)throw new Error('CodeHub MR 已上传，但无法从 codehub-cli 输出中读取 id 和 mr_url。');task.pullRequestUrl=String(mr.mr_url);task.nextStage='DONE';task.progress=100;if(task.governance){task.governance.mrState='PENDING';task.governance.completedAt=new Date().toISOString();}this.change(task,'RESOLVED',`已修复 ${task.governance?.fixedIds?.length??0} 个用例，新增 ${task.governance?.addedIds?.length??0} 个用例，MR 待合并。`);}
