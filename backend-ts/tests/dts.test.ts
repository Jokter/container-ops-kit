...
test('创建请求超时不自动重试，凭据缺失可重新准备',async()=>{
 ...
 try{const service=new DtsTickets(store,async()=>async(method)=>{if(method==='initialize')return{};requests++;throw Error('timeout');});assert.equal((await service.create('a','tester','R27C10')).status,'REVIEW');await service.create('b','tester','R27C10');assert.equal(requests,1);
...
test('缺少目标版本 ID 时在任何远端写入前阻止建单',async()=>{
 const store=new TaskStore(':memory:');let calls=0;
 const service=new DtsTickets(store,async()=>{calls++;throw Error('must not connect');});
 try{await assert.rejects(service.create('a','tester','R28C10'),/V100R028C10B001/);assert.equal(calls,0);assert.equal(store.records('dts-ticket').length,0);}finally{store.close();}
});
