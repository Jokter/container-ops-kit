...
(function () {
  ...
  function deploymentContent() {
    ...
    const candidate = deploymentRuntime.candidates
    const task = deploymentRuntime.task
    const reviewBlockers = deploymentReviewBlockers(task)
    const reviewWarnings = deploymentReviewWarnings(task)
    const canDeploy = canDeployReviewedTask(task)
  ...
  async function deploymentAction() {
    ...
    render(false)
    try {
      deploymentRuntime.task = await request('/api/deployment-tasks/' + task.id + '/execution', {method: 'POST', body: JSON.stringify({expectedRevision: task.revision})})
