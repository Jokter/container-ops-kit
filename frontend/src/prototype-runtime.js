...
import {canConvertFailedQuickDeploymentToReview, canDeployReviewedTask, deploymentReviewBlockers} from './deployment-presentation.js'
...
(function () {
  ...
  async function saveDeploymentValues() {
    ...
    try {
      ...
      showToast(blockers.length ? 'values.yaml 已保存，请处理左侧显示的阻塞项' : 'values.yaml 已保存，现在可以确认配置并部署')
    } catch (error) {
  ...
  style.textContent += '.deployment-blockers,.deployment-ready{display:grid;gap:4px;padding:9px 10px;border-radius:7px;font-size:12px;line-height:1.45}.deployment-blockers{color:#9b3b3b;background:#fff1f1;border:1px solid #f0caca}.deployment-blockers span{overflow-wrap:anywhere}.deployment-ready{color:#176a58;background:#edf9f5;border:1px solid #c6e8dd}'
