...
(function () {
  ...
  function compactServiceEntry(environment, prefix, label) {
    ...
    if (url) {
      ...
      if (parsed.hostname === environment.ip) displayAddress = (parsed.port ? ':' + parsed.port : '同 SSH 主机') + (parsed.pathname === '/' ? '' : parsed.pathname)
    }
    const content = url
      ? '<a href="' + escapeHtml(url) + '" title="' + escapeHtml(address) + '" target="_blank" rel="noopener noreferrer">' + escapeHtml(displayAddress) + '</a>'
  ...
  render = function (resetScroll = true) {
    ...
    const previousDeploymentTerminal = document.querySelector('#deployment-log-terminal')
    const directoryDetailsOpen = document.querySelector('.build-directories')?.open || false
    ...
    const followedLatest = previousTerminal
      ...
      : true
    const previousDeploymentScrollTop = previousDeploymentTerminal?.scrollTop || 0
    const followedLatestDeployment = previousDeploymentTerminal
    ...
    if (nextDeploymentTerminal) nextDeploymentTerminal.scrollTop = followedLatestDeployment
      ? nextDeploymentTerminal.scrollHeight
      : previousDeploymentScrollTop
    const nextDirectoryDetails = document.querySelector('.build-directories')
    if (nextDirectoryDetails) nextDirectoryDetails.open = directoryDetailsOpen
  }

  buildTabs = function () {
    return '<div class="build-navigation"><div><strong>构建工作区</strong><span>创建任务、查看结果与维护配置</span></div><div class="workspace-tabs"><button data-build-tab="single" class="' + (state.buildTab === 'single' ? 'active' : '') + '">单分支构建</button><button data-build-tab="compare" class="' + (state.buildTab === 'compare' ? 'active' : '') + '">双分支对比</button><button data-build-tab="history" class="' + (state.buildTab === 'history' ? 'active' : '') + '">历史任务</button><button data-build-tab="config" class="' + (state.buildTab === 'config' ? 'active' : '') + '">构建配置</button></div></div>'
  }
  ...
  function restoreDeploymentServices(namespace) {
    const environment = environments.find(item => item.id === state.selectedContainerEnvironment) || environments.find(item => item.type === 'container')
  ...
  function deploymentServiceRows(task) {
    ...
    return Object.entries(task.services).map(([name, service]) => {
      ...
      const selectable = task.mode === 'REVIEW' && task.status === 'AWAITING_REVIEW'
      const checkbox = selectable ? '<input type="checkbox" name="reviewDeploymentService" value="' + escapeHtml(name) + '" ' + (deploymentRuntime.selectedServices.has(name) ? 'checked' : '') + ' aria-label="选择部署 ' + escapeHtml(name) + '">' : ''
      return '<div class="deployment-service-row ' + (deploymentRuntime.activeService === name ? 'active' : '') + '">' + checkbox + '<button type="button" data-deployment-service="' + escapeHtml(name) + '"><span><strong>' + escapeHtml(name) + '</strong><small>' + escapeHtml(errors || ((service?.replaceItems?.length || 0) + ' 项自动替换')) + '</small></span></button><span class="badge ' + presentation[1] + '">' + presentation[0] + '</span></div>'
    }).join('')
  }
  ...
  function deploymentDetails() {
    ...
    if (!service) return '<section class="panel deployment-editor-panel"><div class="panel-body"><div class="environment-empty">分析完成后可查看替换项并编辑 values.yaml。</div></div></section>'
    ...
    return '<section class="panel deployment-editor-panel"><div class="panel-head deployment-editor-head"><div><h2>' + escapeHtml(service.service) + '</h2><p>values.yaml · 未解析镜像：' + escapeHtml((service.unresolvedImages || []).join('、') || '无') + '</p></div><button class="button primary small" data-deployment-save-values>保存 values.yaml</button></div><div class="panel-body deployment-editor-body"><details class="deployment-replacements" ' + ((service.replaceItems || []).length ? 'open' : '') + '><summary><span>自动替换预览</span><span class="badge">' + (service.replaceItems || []).length + ' 项</span></summary><div class="replacement-list">' + replaces + '</div></details><div class="deployment-code-head"><span>values.yaml</span><small>可直接编辑，保存后会重新校验配置</small></div><textarea id="deployment-values" class="deployment-values mono" spellcheck="false">' + escapeHtml(service.values || '') + '</textarea></div></section>'
  }
  ...
  function deploymentContent() {
    ...
    const candidate = deploymentRuntime.candidates
    const task = deploymentRuntime.task
    const selectedReviewTask = task?.status === 'AWAITING_REVIEW' ? {...task, services: Object.fromEntries(Object.entries(task.services || {}).filter(([name]) => deploymentRuntime.selectedServices.has(name)))} : task
    const reviewBlockers = deploymentReviewBlockers(selectedReviewTask)
    const reviewWarnings = deploymentReviewWarnings(selectedReviewTask)
    const canDeploy = deploymentRuntime.selectedServices.size > 0 && canDeployReviewedTask(selectedReviewTask)
    const progress = deploymentProgress(task, deploymentRuntime.logs)
  ...
  async function deploymentAction() {
    ...
    render(false)
    try {
      deploymentRuntime.task = await request('/api/deployment-tasks/' + task.id + '/execution', {method: 'POST', body: JSON.stringify({expectedRevision: task.revision, services})})
  ...
  async function saveDeploymentValues() {
    ...
    try {
      ...
      render(false)
      const blockers = deploymentReviewBlockers(deploymentRuntime.task)
      const warnings = deploymentReviewWarnings(deploymentRuntime.task)
      showToast(blockers.length ? 'values.yaml 已保存，请处理左侧显示的阻塞项' : warnings.length ? 'values.yaml 已保存；可选版本未解析，但不阻塞审阅部署' : 'values.yaml 已保存，现在可以确认配置并部署')
    } catch (error) {
  ...
  document.addEventListener('change', function (event) {
    if (event.target.name === 'reviewDeploymentService') {
    ...
    if (event.target.id === 'deployment-namespace') {
      deploymentRuntime.task = null
      deploymentRuntime.eventSource?.close()
      return loadDeploymentCandidates(event.target.value).then(() => restoreDeploymentServices(event.target.value))
    }
  ...
  document.addEventListener('click', function (event) {
    ...
    if (event.target.closest?.('[data-copy-deployment-logs]')) {
      const lines = deploymentRuntime.logs.slice(-500)
      if (!lines.length) return showToast('没有可复制的部署日志')
      return copyText(lines.map(item => item.time + ' [' + item.stage + '] ' + (item.service ? item.service + ' · ' : '') + item.message).join('\n'), '部署日志已复制')
    }
  ...
  style.textContent += '.build-navigation{display:flex;align-items:center;justify-content:space-between;gap:18px;margin-bottom:16px;padding:12px 14px;border:1px solid var(--line);border-radius:10px;background:var(--surface)}.build-navigation>div:first-child{display:grid;gap:2px}.build-navigation>div:first-child>span{color:var(--muted);font-size:12px}.build-navigation .workspace-tabs{margin:0;padding:3px;border-radius:8px;background:var(--surface-soft)}.build-navigation .workspace-tabs button{padding:7px 11px}.build-input-panel .field{padding:10px;border:1px solid var(--line);border-radius:9px;background:var(--surface-soft)}.build-input-panel .field input,.build-input-panel .field select{background:var(--surface)}.build-input-panel .field[style]{padding:0;border:0;background:transparent}.build-layout{grid-template-columns:minmax(360px,440px) minmax(0,1fr)}.build-storage{margin:0 0 16px;background:var(--surface);box-shadow:0 1px 2px rgba(25,39,48,.03)}@media(max-width:900px){.build-navigation{align-items:stretch;flex-direction:column}.build-navigation .workspace-tabs{overflow:auto}.build-layout{grid-template-columns:1fr}}'
  ...
  style.textContent += '.deployment-blockers,.deployment-ready,.deployment-warnings{display:grid;gap:4px;padding:9px 10px;border-radius:7px;font-size:12px;line-height:1.45}.deployment-blockers{color:#9b3b3b;background:#fff1f1;border:1px solid #f0caca}.deployment-blockers span,.deployment-warnings span{overflow-wrap:anywhere}.deployment-ready{color:#176a58;background:#edf9f5;border:1px solid #c6e8dd}.deployment-warnings{color:#8a5b10;background:#fff8e8;border:1px solid #efdba8}'
  style.textContent += '.deployment-overview{margin-top:16px}.deployment-overview .panel-head p,.deployment-editor-head p,.deployment-log-panel .panel-head p{margin-top:4px;color:var(--muted);font-size:11px}.deployment-overview .deployment-actions>.button{justify-self:start}.deployment-progress{display:grid;gap:9px;margin-bottom:14px}.deployment-progress div{display:flex;justify-content:space-between;gap:10px;font-size:13px}.deployment-progress span{color:var(--muted)}.deployment-progress progress{width:100%;height:10px;overflow:hidden;border:0;border-radius:99px;accent-color:var(--brand)}.deployment-workspace{display:grid;grid-template-columns:minmax(240px,280px) minmax(0,1fr);gap:16px;margin-top:16px;align-items:start}.deployment-workspace.quick{grid-template-columns:1fr}.deployment-services-panel .panel-head>span{color:var(--muted);font-size:12px}.deployment-services-panel .panel-body{padding:10px}.deployment-service-list{max-height:680px;overflow:auto}.deployment-service-row{border-color:transparent;background:var(--surface-soft)}.deployment-service-row:hover{border-color:var(--line);background:var(--surface)}.deployment-service-row.active{border-color:var(--brand);background:var(--surface);box-shadow:0 0 0 1px var(--brand)}.deployment-service-row>input{flex:0 0 auto}.deployment-service-row>button{display:block;min-width:0;flex:1;border:0;padding:0;color:inherit;background:transparent;text-align:left}.deployment-service-row>button>span{display:grid;gap:3px;min-width:0}.deployment-editor-panel{min-width:0;overflow:hidden}.deployment-editor-head{align-items:center}.deployment-editor-body{padding:0}.deployment-replacements{border-bottom:1px solid var(--line);background:var(--surface-soft)}.deployment-replacements>summary{display:flex;align-items:center;justify-content:space-between;padding:11px 16px;cursor:pointer;list-style:none;font-size:12px;font-weight:600}.deployment-replacements>summary::-webkit-details-marker{display:none}.deployment-replacements .replacement-list{max-height:220px;padding:0 10px 10px}.deployment-code-head{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:11px 16px;color:var(--muted);font-size:12px}.deployment-code-head>span{color:var(--text);font-weight:600}.deployment-values{display:block;min-height:560px;max-height:72vh;resize:vertical;border:0;border-top:1px solid #31434e;border-radius:0;padding:16px 18px;background:#16232b;color:#d8e2e7;font-size:13px;line-height:1.65;tab-size:2;outline:none}.deployment-values:focus{box-shadow:inset 3px 0 0 var(--brand)}.deployment-log-panel{margin-top:16px}.deployment-log-actions{display:flex;align-items:center;gap:10px;color:var(--muted);font-size:12px}.deployment-log-terminal{min-height:220px;max-height:460px;overflow:auto;padding:12px 14px}.deployment-log-line{display:grid;grid-template-columns:76px 68px minmax(0,1fr);gap:10px;min-height:25px;align-items:start}.deployment-log-line>b{color:#7f96a3;font-weight:500}.deployment-log-stage{color:#63c5a8}.deployment-log-empty{color:#7f96a3}.replacement-row{grid-template-columns:minmax(90px,.7fr) minmax(120px,.8fr) minmax(140px,1fr) minmax(140px,1fr)}@media(max-width:1000px){.deployment-workspace{grid-template-columns:1fr}.deployment-service-list{max-height:300px}.deployment-values{min-height:480px}}@media(max-width:700px){.deployment-log-line{grid-template-columns:66px 55px minmax(0,1fr);gap:6px}.deployment-code-head small{display:none}.deployment-editor-head{align-items:flex-start}.deployment-editor-head .button{white-space:nowrap}.deployment-values{min-height:420px;padding:13px;font-size:12px}}'
