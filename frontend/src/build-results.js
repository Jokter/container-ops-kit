...
export function createBuildResults({request,render,toast,escapeHtml}) {
  const ui={id:'',task:null,result:null,error:'',loading:false,query:'',page:1,pageSize:20,filter:'ALL',selected:new Set(),service:'',file:'',downloading:false,generation:0}
  ...
  function pageRows(rows){
    ui.page=Math.max(1,Math.min(ui.page,Math.ceil(rows.length/ui.pageSize)||1))
    return rows.slice((ui.page-1)*ui.pageSize,ui.page*ui.pageSize)
  }
  function pager(total){
    const pages=Math.ceil(total/ui.pageSize)||1,start=total?(ui.page-1)*ui.pageSize+1:0,end=Math.min(ui.page*ui.pageSize,total)
    return `<div class="br-pagination"><span>显示 ${start}–${end} / ${total} 个服务</span><label>每页 <select data-br-page-size aria-label="每页服务数量">${[20,50,100].map(n=>`<option value="${n}" ${n===ui.pageSize?'selected':''}>${n}</option>`).join('')}</select> 项</label><button class="button small" data-br-page="${ui.page-1}" ${ui.page===1?'disabled':''}>上一页</button><span aria-live="polite">${ui.page} / ${pages}</span><button class="button small" data-br-page="${ui.page+1}" ${ui.page===pages?'disabled':''}>下一页</button></div>`
  }
  async function load(task,force=false){
    if(task.status!=='SUCCEEDED')return
    if(ui.id!==task.id){ui.id=task.id;ui.task=task;ui.result=null;ui.error='';ui.loading=false;ui.query='';ui.page=1;ui.filter='ALL';ui.selected.clear();ui.service='';ui.file='';ui.generation++}
    ui.task=task
  ...
  function detail(){
    const rows=pageRows(services());let row=rows.find(r=>r.service===ui.service)||rows[0]
  ...
  document.addEventListener('click',event=>{
    const button=event.target.closest?.('[data-br-download],[data-br-batch],[data-br-filter],[data-br-service],[data-br-file],[data-br-retry],[data-br-export],[data-br-page]');if(!button)return
    ...
    if(button.dataset.brPage!==undefined){if(button.disabled)return;ui.page=Number(button.dataset.brPage);ui.service='';ui.file=''}
    if(button.dataset.brFilter){ui.page=1;ui.filter=button.dataset.brFilter;ui.service=services()[0]?.service||'';ui.file=''}
  ...
  document.addEventListener('input',event=>{if(event.target.id!=='br-search')return;ui.query=event.target.value;ui.page=1;const cursor=event.target.selectionStart;ui.service=services()[0]?.service||'';ui.file='';render(false);const input=document.getElementById('br-search');input?.focus();input?.setSelectionRange(cursor,cursor)})
  document.addEventListener('change',event=>{if(event.target.hasAttribute('data-br-page-size')){ui.pageSize=Number(event.target.value);ui.page=1;ui.service='';ui.file='';render(false);return}const name=event.target.dataset.brCheck;if(name!==undefined){event.target.checked?ui.selected.add(name):ui.selected.delete(name);render(false)}if(event.target.hasAttribute('data-br-all')){pageRows(packages()).forEach(row=>event.target.checked?ui.selected.add(row.service):ui.selected.delete(row.service));render(false)}})
  return {html,load}
}
