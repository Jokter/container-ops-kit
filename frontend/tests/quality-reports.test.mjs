...
test('任务进度使用列表与详情布局并提供清晰筛选和操作命名',async()=>{
 const tasks=[{id:'running',repository:'RunRepo',reportVersion:'R27C10',status:'REPAIRING',nextStage:'REPAIR',progress:45,workspaceRoot:'/tmp',repairBranch:'repair-run',message:'正在执行 Pi 修复。'},{id:'waiting',repository:'WaitRepo',reportVersion:'R27C00',status:'WAITING_EXTERNAL',nextStage:'VERIFY',progress:70,workspaceRoot:'/tmp',repairBranch:'repair-wait',message:'验证失败，请处理。'}];
 const dom=page(path=>path==='/api/auto-ut/tasks'?tasks:undefined);try{const d=open(dom,'auto-ut');await pause();d.querySelector('[data-qw-auto-tab="tasks"]').click();assert.equal(d.querySelectorAll('.ut-task-row').length,2);assert.equal(d.querySelectorAll('.ut-task-timeline>div').length,6);assert.deepEqual([...d.querySelectorAll('[data-auto-ut-detail-tab]')].map(x=>x.textContent),['执行过程','Pi 对话','工具调用','完整日志']);d.querySelector('[data-auto-ut-select="waiting"]').click();assert.match(d.querySelector('.ut-task-detail').textContent,/重试当前步骤/);const status=d.querySelector('#qw-task-status');status.value='waiting';status.dispatchEvent(new dom.window.Event('change'));assert.equal(d.querySelectorAll('.ut-task-row').length,1);assert.match(d.querySelector('.ut-task-row').textContent,/WaitRepo/);assert.doesNotMatch(d.body.textContent,/删除任务和代码|打开 CodeHub MR/);}finally{dom.window.close();}
});
